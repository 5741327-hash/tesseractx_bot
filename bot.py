...
import random 
import asyncio # Импортируем для использования задержки
...
# КОНСТАНТА: Максимальная безопасная длина ПЕРВОЙ ЧАСТИ поста (тизера).
# Устанавливается в 200 символов, чтобы служить коротким "заголовком-крючком".
MAX_TEASER_LENGTH = 200 
...
def generate_ai_content(title, raw_text):
    ...
    Запрашивает разделение поста на тизер (часть 1) и основной текст (часть 2).
    """
    
    # ОБНОВЛЕНИЕ ПРОМТА: Указываем на короткий тизер и основной текст во второй части
    system_prompt = (
        ...
        f"<b>ПЕРВАЯ ЧАСТЬ</b> должна быть коротким, интригующим заголовком-крючком (тизером) и иметь длину <b>НЕ БОЛЕЕ {MAX_TEASER_LENGTH} СИМВОЛОВ</b>. Она должна предварять основной текст. "
        "<b>ВТОРАЯ ЧАСТЬ</b> должна содержать ВЕСЬ основной, структурированный текст статьи. Обязательно структурируй ее, используя <b>полужирные заголовки</b> или <b>ключевые фразы</b>. "
        ...
        "[ПОСТ ЧАСТЬ 1]\n"
        "Текст первой части (тизер)...\n\n"
        "[ПОСТ ЧАСТЬ 2]\n"
        "Текст второй части (основной текст)...\n\n"
        "[DALL-E PROMPT]\n"
    ...
    try:
        ...
        if post1_match and post2_match and prompt_match:
            ...
            # Принудительное обрезание ЧАСТИ 1, если AI ошибся
            if len(post_part_1) > MAX_TEASER_LENGTH:
                # Перемещаем обрезанный хвост в часть 2
                overflow = post_part_1[MAX_TEASER_LENGTH:]
                post_part_1 = post_part_1[:MAX_TEASER_LENGTH] + "..." 
...
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- ПРОВЕРКА ЧАСТИ 1 ---
    if len(post_part_1) > MAX_TEASER_LENGTH: # Используем новую константу
        await update.message.reply_text(f"⚠️ <b>Внимание:</b> Длина <b>Первой части (тизера)</b> превысила целевой лимит ({MAX_TEASER_LENGTH} символов) и была принудительно обрезана. Длина после обрезки: {len(post_part_1)}.", parse_mode='HTML')
    # ----------------------------------------------------
    ...
    # Формируем подпись для ЧАСТИ 1 (короткий тизер)
    caption_draft = f"<b>[Черновик]</b>\n\n{post_part_1}\n\n<i>(Основной текст в следующем сообщении)</i>\n\n/publish для публикации"
...
async def publish_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # ШАГ 1: Отправляем фото с ЧАСТЬЮ 1 (тизером)
