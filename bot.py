...
try:
    ...
    # 🌟 ИСПРАВЛЕНИЕ #1: Очищаем и преобразуем ADMIN_ID в число (INT)
    ADMIN_ID_STR = os.getenv("ADMIN_ID")
    if ADMIN_ID_STR:
        ADMIN_ID = int(ADMIN_ID_STR.strip()) # strip() удаляет лишние пробелы/переводы строк
    else:
        ADMIN_ID = None
...
def restricted(func):
    ...
    async def wrapped(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        # 🌟 ИСПРАВЛЕНИЕ #2: Используем INT для сравнения. 
        user_id = update.effective_user.id 
        if user_id != ADMIN_ID: # Сравниваем INT с INT
...
def generate_ai_content(title, raw_text):
    ...
    system_prompt = (
        ...
        "3. Форматирование: Используй HTML-теги <b></b> для жирного, <i></i> для курсива, и **<b><i> для жирного курсива ключевых фраз</b></i>**."
        
        "**СТРОГО СОБЛЮДАЙ СЛЕДУЮЩУЮ СТРУКТУРУ ПОСТА (Не включай в ответ заголовки, такие как 'КРАТКАЯ СУТЬ' или 'КОНТЕКТ'):**"
