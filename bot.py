...
def generate_ai_content(title, raw_text):
    ...
    # ОБНОВЛЕНИЕ ПРОМТА: Требование использовать HTML-форматирование и включить ссылку
    system_prompt = (
        ...
        f"ОБЩАЯ ДЛИНА ГОТОВОГО ПОСТА НЕ ДОЛЖНА ПРЕВЫШАТЬ {MAX_POST_LENGTH} СИМВОЛОВ (включая пробелы и эмодзи)! " 
        ...
        "[ПОСТ]\n"
        "Текст готового поста с HTML-тегами...\n\n"
        "[DALL-E PROMPT]\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        parse_mode='HTML' # Используем HTML для форматирования инструкции
    )
...
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 4. Сохраняем черновик поста (включая URL) и отправляем его администратору
    global draft_post
    draft_post = {'text': post_text, 'image_url': image_url, 'source_url': url}
    
    # Добавляем ссылку на источник к черновику
...
async def handle_manual_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    global draft_post
    draft_post = {'text': post_text, 'image_url': image_url, 'source_url': None}
    
    # В ручном режиме просто помечаем как черновик
    caption_draft = f"<b>[Черновик]</b>\n\n{post_text}\n\n/publish для публикации"
...
async def publish_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    final_text = draft_post['text']
    source_url = draft_post.get('source_url')
    
    # Добавляем ссылку на источник к финальному посту
    if source_url:
