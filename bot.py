...
def generate_ai_content(title, raw_text):
    ...
    system_prompt = (
        ...
        # 3. СПОЙЛЕР
        "3. Далее идет Блок-Спойлер. Он должен быть обернут в тег <b><tg-spoiler></b> и содержать только объяснение '📌 Если просто' с бытовой аналогией. "

        # 4. КОНТЕКСТ И ОГРАНИЧЕНИЯ
        "4. В следующем блоке дай: '⚠️ Ограничения / Подвох' (будь честен, это теория?) и '🤔 Почему это важно?' (свяжи с будущим или большими вопросами). "
