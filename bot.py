...
def generate_ai_content(title, raw_text):
    ...
    system_prompt = (
        "Ты — редактор популярного научно-популярного Telegram-канала «Горизонт событий». Твоя задача — превращать сложные научные новости в цепляющие, вирусные посты для широкой аудитории."
        ...
        "1. Живой и разговорный: Используй провокационные вопросы и яркие аналогии из поп-культуры. "
        "2. Эмоциональный: Добавляй 2-3 релевантных эмодзи, особенно в начале. "
        "3. Форматирование: Используй HTML-теги <b></b> для жирного, <i></i> для курсива, и **<b><i> для жирного курсива ключевых фраз</b></i>**." # <-- ИЗМЕНЕНИЕ ЗДЕСЬ
        ...
        # 2. КРАТКАЯ СУТЬ
        "2. Кратко (2-3 абзаца) изложи суть открытия простыми словами, используя метафору. Упомяни область науки и, если важно, институт. "

        # 3. СПОЙЛЕР
        "3. Далее идет Блок-Спойлер. Он должен быть обернут в тег <b></b> и содержать только объяснение '📌 Если просто' с бытовой аналогией. "
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Используем Markdown для простого статичного текста
...
async def handle_manual_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Эта проверка должна быть здесь, чтобы отфильтровать короткие сообщения,
    # которые не являются командами, но и не являются статьями.
    if len(raw_text) < 500:
        return
...
def main():
    ...
    # filters.COMMAND исключает /start, /publish и т.д. - это должно устранить двойные срабатывания.
    app.add_handler(MessageHandler(
        filters.TEXT 
        & ~filters.COMMAND 
